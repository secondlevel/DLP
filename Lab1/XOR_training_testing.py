...
if __name__ == "__main__":
    ...
    x_test, y_test = generator_XOR_easy(n = data_number)
    x_test = x_test.astype('float32')
    y_test = y_test.astype('float32').reshape(data_number*2-1,)
    
    y_pred, _ = forward_propagation(np.transpose(x_test), params_values, TwoLayerNet)
    pred_label_test,acc_test = get_accuracy(y_pred, np.transpose(y_test.reshape((y_test.shape[0],))))
